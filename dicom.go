...
// OrganizeSeriesFiles sorts and renames DICOM files within a single series directory.
// It assumes all provided files belong to the same series and are in the same directory.
func OrganizeSeriesFiles(dicomFiles []*DicomFile, seriesDir string) error {
...
// OrganizeDicomFiles takes a list of DICOM files, groups them by series,
// creates a directory for each series, and then organizes the files within.
func OrganizeDicomFiles(files []*DicomFile, outputDir string) error {
	...
	for seriesUID, dicomFiles := range series {
		...
		if err := OrganizeSeriesFiles(dicomFiles, seriesDir); err != nil {
			return fmt.Errorf("could not organize files in series directory %s: %v", seriesDir, err)
		}
	}
	return nil
}
