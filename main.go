...
func main() {
	...
	if options.Version {
					...
					if ctx.Options.Meta {
						...
						if fileInfo.NeedsDownload(ctx.Options.Output, ctx.Options.Force, ctx.Options.NoDecompress) {
							if err := fileInfo.Download(ctx.Options.Output, ctx.HTTPClient, ctx.AuthToken, ctx.Options); err != nil {
								...
								// We identify s5cmd downloads by checking if S5cmdManifestPath is set.
								if fileInfo.S5cmdManifestPath != "" {
									// The actual downloaded file path is now stored in FileName by the downloadFromS3 function.
		...
		if len(dicomFileChan) > 0 {
			...
			// Using a map to collect unique series UIDs
			seriesUIDMap := make(map[string]struct{})
			...
			for path := range dicomFileChan {
				...
				// Add the series UID to the map
				seriesUIDMap[dicomFile.SeriesUID] = struct{}{}
			}

			// Convert map keys to a slice for fetching metadata
			for uid := range seriesUIDMap {
				seriesUIDs = append(seriesUIDs, uid)
			}

			// Fetch metadata in bulk for all series found in the downloaded DICOMs.
			// This is useful for getting consistent metadata for all series.
			if len(seriesUIDs) > 0 {
				_ = FetchMetadataForSeriesUIDs(seriesUIDs, client, token, options)
			}
			...
			fmt.Fprintln(os.Stderr, "DICOM file organization complete.")
		}
